...
## [0.4.0]
### 新增
- **语音播报改用 Edge TTS（默认）**：把某类提醒设为“语音”后，由**本机 DSH 主进程**调用微软 Edge 朗读服务合成 MP3，浏览器只负责播放，中文不再是机械音。
  - 浏览器不能直连该服务：页面发起的 WebSocket 携带页面 `Origin` 会被 **403** 拒绝，且它要求的 User-Agent 浏览器改不了；故主机端新增 `GET /dsh-alert-sound/tts.mp3`（内存缓存、参数校验、拒绝跨站请求）。
  - 设置页新增 **语音引擎**（Edge 语音 / 浏览器语音，可完全离线）与 **音色**（自动 + 8 个候选：晓晓/晓伊/云希/云扬/Ava/Emma/Andrew/Brian）+ **试听**；默认 `自动`＝中文晓晓、英文 Ava。
  - **预取**：启用“语音”的提醒类型，固定短句在启动后于后台合成并缓存，提醒到来立即出声。
  - **三级兜底**：Edge 语音 → 浏览器语音 → 该类提示音，保证不静音；连续 3 次合成失败即停用 Edge 引擎并在设置页提示（控制台给出原因）。
- `test/host.test.mjs`：主机端离线测试（参数校验、SSML 转义、同源校验、错误码），联网用例默认跳过；`npm test` 与 `prepublishOnly` 都会运行。
### 变更
- 新增运行时依赖 `ws`（仅主机端的 WebSocket 客户端使用）。
- README（中/英）与 `SECURITY.md` 补充**联网说明**：只有“语音”音色会把要朗读的文本（提醒短句，或开启“朗读输出”时截断到 400 字的回复）发送给微软朗读服务；其它音色与“浏览器语音”引擎完全不联网。
