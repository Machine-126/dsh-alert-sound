...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULTS = { volume: 0.7, scope: "all", repeatMs: 20000, types: DEFAULT_TYPES };
        ...
        for (const id of Object.keys(byId)) {
          ...
          if (p) {
            if (pending !== undefined && pending !== p.pending) {
              if (pending === "approval") {
                const pd = pendingDetail(detailOf(id));
                if (shouldFire(id, "approval")) { playType("approval", pd.approval); emit("approval"); startRepeat(id, "approval", Infinity); }
              } else if (pending === "question") {
                const pd = pendingDetail(detailOf(id));
                if (shouldFire(id, "question")) { playType("question", pd.question); emit("question"); startRepeat(id, "question", Infinity); }
              } else {
                stopRepeat(id);
              }
            }
