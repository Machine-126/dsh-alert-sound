  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        "read": "朗读输出", "read.hint": "需配合“语音”音色：开启后，完成时把助手最后的回复念出来（较长时只念开头）", "failed.hint": "一轮运行中途出错时提醒",
        ...
        "preview": "试听", "upload": "上传", "sep": "：", "reset": "恢复默认设置", "reset.hint": "恢复全部选项为默认值（已上传的自定义音色保留）",
      ...
      en: {
        ...
        "read": "Read-aloud", "read.hint": "Works with the “Voice” sound: on completion, speak the assistant's final reply (truncated if long)", "failed.hint": "Alerts when a run errors mid-turn",
        ...
        "preview": "Preview", "upload": "Upload", "sep": ": ", "reset": "Restore defaults", "reset.hint": "Reset all options to defaults (uploaded custom sounds are kept)",
    ...
    function playPattern(sound) {
      ...
      enqueuePlay((release) => {
        ...
        const schedule = () => {
          if (fired) return;
          fired = true;
          if (!audioCtx || audioCtx !== ac || !master) return;
        ...
        else { try { ac.resume().then(schedule, schedule); } catch (e) { schedule(); } }
        later(schedule, 120); // 兜底：resume 迟迟不 resolve 时也排程
        later(release, patternMs(pattern) + 80);
      });
    }
    function speak(text) {
      enqueuePlay((release) => {
    ...
    function apply(ctx) {
      ...
      function SettingsPanel(props) {
        ...
        return react.createElement("div", { style: cardStyle },
          ...
          react.createElement("div", { style: rowStyle },
            react.createElement("button", { style: btnStyle, onClick: () => commit(deepMerge(DEFAULTS, {})) }, t("reset")),
