...
//
// Speech has two engines: “Edge” (default) fetches MP3 from the host half's
// /dsh-alert-sound/tts.mp3 route, and “Browser” uses window.speechSynthesis.
// Edge failure falls back to browser speech, and either failure falls back to
// the kind's tone, so an alert always makes a sound.
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULTS = { volume: 0.7, scope: "all", repeatMs: 20000, notifyEnabled: false, readOutput: false, stallMs: 0, showToast: false, voiceRate: 1, voiceEngine: "edge", edgeVoice: "auto", dndEnabled: false, dndStart: 22, dndEnd: 8, lang: "auto", types: DEFAULT_TYPES };
    ...
    // ---- Edge 语音：宿主半合成 MP3，浏览器只负责播放 ----
    // 浏览器不能直连微软朗读服务（页面 Origin 会被 403 拒绝），所以宿主半暴露
    // EDGE_ROUTE，前端取回音频字节再播；取不到就回退浏览器 TTS 与提示音。
    const EDGE_ROUTE = "/dsh-alert-sound/tts.mp3";
    const EDGE_VOICE_AUTO = { zh: "zh-CN-XiaoxiaoNeural", en: "en-US-AvaMultilingualNeural" };
    const EDGE_VOICES = [
      { id: "zh-CN-XiaoxiaoNeural", zh: "晓晓 · 女声 · 温暖自然", en: "Xiaoxiao · female · warm, natural" },
      { id: "zh-CN-XiaoyiNeural", zh: "晓伊 · 女声 · 活泼明亮", en: "Xiaoyi · female · lively, bright" },
      { id: "zh-CN-YunxiNeural", zh: "云希 · 男声 · 阳光", en: "Yunxi · male · sunny" },
      { id: "zh-CN-YunyangNeural", zh: "云扬 · 男声 · 专业播报", en: "Yunyang · male · professional news" },
      { id: "en-US-AvaMultilingualNeural", zh: "Ava · 女声 · 亲切自然（多语言）", en: "Ava · female · warm, expressive (multilingual)" },
      { id: "en-US-EmmaMultilingualNeural", zh: "Emma · 女声 · 明快清晰（多语言）", en: "Emma · female · cheerful, clear (multilingual)" },
      { id: "en-US-AndrewMultilingualNeural", zh: "Andrew · 男声 · 温暖自信（多语言）", en: "Andrew · male · warm, confident (multilingual)" },
      { id: "en-US-BrianMultilingualNeural", zh: "Brian · 男声 · 随和自然（多语言）", en: "Brian · male · casual, sincere (multilingual)" },
    ];
    const EDGE_FETCH_TIMEOUT_MS = 8000;
    // 连续失败到这个次数就停用 Edge 引擎（路由未就绪时不必每条提醒都白等）。
    const EDGE_FAILURE_LIMIT = 3;
    const EDGE_CACHE_MAX = 48;
    ...
    const I18N = {
      zh: {
        ...
        "engine": "语音引擎", "engine.edge": "Edge 语音（更自然，需联网，默认）", "engine.browser": "浏览器语音（完全离线）",
        "edge.voice": "音色", "edge.auto": "自动（中文→晓晓 / 英文→Ava）",
        "edge.unavailable": "⚠️ Edge 语音不可用，已改用浏览器语音",
        "edge.hint": "Edge 语音由本机 DSH 主进程调用微软朗读服务合成，首次 1 秒左右，固定提醒会后台预取。开启期间只有“要朗读的文本”会离开本机；切成“浏览器语音”即可完全不联网。",
        ...
        "hint": "选“语音”会用朗读代替提示音：默认走 Edge 语音，不可用时回退浏览器语音。", "stalled.detail": "长时间未进展",
      },
      en: {
        ...
        "engine": "Voice engine", "engine.edge": "Edge speech (more natural, needs network; default)", "engine.browser": "Browser speech (fully offline)",
        "edge.voice": "Voice", "edge.auto": "Auto (Chinese → Xiaoxiao / English → Ava)",
        "edge.unavailable": "⚠️ Edge speech unavailable; using browser speech",
        "edge.hint": "Edge speech is synthesized by the local DSH host through Microsoft's read-aloud service (about a second on first use; fixed alerts are prefetched). While it is on, only the text being spoken leaves this machine — switch to browser speech for a fully offline mode.",
        ...
        "hint": "Choosing “Voice” speaks instead of a tone: Edge speech by default, browser speech when it is unavailable.", "stalled.detail": "No progress for a while",
      },
    };
    ...
    // ---- Edge 语音引擎：音色、缓存与预取 ----
    const edgeAudio = new Map(); // 合成键 → objectURL
    let edgeFailures = 0;
    let edgeDisabled = false;
    let edgeStatus = "";
    let edgePrefetchKey = "";
    function edgeVoiceId() {
      const chosen = settings.edgeVoice || "auto";
      if (chosen !== "auto" && chosen) return chosen;
      return resolveLang() === "en" ? EDGE_VOICE_AUTO.en : EDGE_VOICE_AUTO.zh;
    }
    // 语速在宿主半按百分比换算（0.7 → -30%）。
    function edgeRatePercent() {
      return Math.max(-50, Math.min(100, Math.round(((settings.voiceRate || 1) - 1) * 100)));
    }
    function edgeCachePut(key, url) {
      edgeAudio.set(key, url);
      while (edgeAudio.size > EDGE_CACHE_MAX) {
        const oldest = edgeAudio.keys().next();
        if (oldest.done) break;
        const stale = edgeAudio.get(oldest.value);
        edgeAudio.delete(oldest.value);
        try { URL.revokeObjectURL(stale); } catch (e) { /* 已经释放过 */ }
      }
    }
    function edgeAudioUrl(text) {
      const voice = edgeVoiceId();
      const rate = edgeRatePercent();
      const key = voice + "|" + rate + "|" + text;
      const hit = edgeAudio.get(key);
      if (hit) return Promise.resolve(hit);
      const url = EDGE_ROUTE
        + "?text=" + encodeURIComponent(text)
        + "&voice=" + encodeURIComponent(voice)
        + "&rate=" + rate;
      const controller = typeof AbortController === "function" ? new AbortController() : null;
      later(() => { if (controller) { try { controller.abort(); } catch (e) {} } }, EDGE_FETCH_TIMEOUT_MS);
      return fetch(url, controller ? { signal: controller.signal } : undefined).then((res) => {
        if (!res.ok) throw new Error("HTTP " + res.status);
        return res.blob();
      }).then((blob) => {
        if (!blob || blob.size === 0) throw new Error("空音频");
        const objectUrl = URL.createObjectURL(blob);
        edgeCachePut(key, objectUrl);
        return objectUrl;
      });
    }
    function markEdgeFailure(error) {
      edgeFailures += 1;
      edgeStatus = error && error.message ? error.message : String(error);
      if (edgeFailures === 1) { try { console.warn("[dsh-alert-sound] Edge 语音合成失败，回退浏览器语音：", edgeStatus); } catch (e) {} }
      if (edgeFailures >= EDGE_FAILURE_LIMIT && !edgeDisabled) {
        edgeDisabled = true;
        notifySettings();
      }
    }
    // 预取固定提醒的音频：Edge 合成首次要 1 秒左右，赶在提醒到来之前取好，
    // 提醒本身就能立刻出声。语言/音色/语速或启用项变化时重新预取。
    function scheduleEdgePrefetch() {
      if (settings.voiceEngine !== "edge" || edgeDisabled) return;
      const kinds = KINDS.filter(kind => {
        const cfg = settings.types && settings.types[kind];
        return !!(cfg && cfg.enabled && cfg.sound === "voice");
      });
      const key = [edgeVoiceId(), edgeRatePercent(), resolveLang(), kinds.join(",")].join("|");
      if (key === edgePrefetchKey) return;
      edgePrefetchKey = key;
      later(() => {
        if (settings.voiceEngine !== "edge" || edgeDisabled) return;
        for (const kind of kinds) edgeAudioUrl(t(kind)).catch(markEdgeFailure);
      }, 1200);
    }
    // 试听当前音色：直接走 Edge 引擎说一句样例，便于挑选音色。
    function previewEdgeVoice() {
      const text = resolveLang() === "en" ? "Alerts are ready. This is how I sound." : "提醒已就绪，这就是我说话的声音。";
      enqueuePlay((release) => {
        let released = false;
        const done = () => { if (released) return; released = true; release(); };
        const tone = () => {
          const pattern = PATTERNS.ding;
          if (pattern) schedulePatternNow(pattern, done); else done();
        };
        if (edgeDisabled) { speakBrowser(text, done, tone); return; }
        speakEdge(text, done, () => speakBrowser(text, done, tone));
      });
    }
    // 语音朗读。三层兜底：Edge 语音 → 浏览器 TTS → 该类提示音，
    // 保证任何一层失败都还有声音。
    function speak(text, fallbackSound) {
      enqueuePlay((release) => {
        ...
        const browser = () => speakBrowser(text, done, tone);
        if (settings.voiceEngine === "edge" && !edgeDisabled) speakEdge(text, done, browser);
        else browser();
      });
    }
    function speakEdge(text, done, onFail) {
      let settled = false;
      let player = null;
      const fail = (error) => {
        if (settled) return;
        settled = true;
        markEdgeFailure(error);
        onFail();
      };
      const ok = () => {
        if (settled) return;
      ...
      // 播放看门狗：拿到音频却没真正开始（例如被自动播放策略拦下）就走兜底。
      later(() => {
        if (!settled && player && player.paused && !player.currentTime) fail(new Error("播放未开始"));
      }, 2500);
      // 总兜底：onended 未触发时也不卡住队列。
      later(ok, Math.max(6000, Math.min(30000, (text ? text.length : 0) * 200 + 4000)));
      edgeAudioUrl(text).then((url) => {
        if (settled) return;
        try {
          player = new Audio(url);
          player.volume = Math.min(1, Math.max(0, settings.volume));
          player.onended = ok;
          player.onerror = () => fail(new Error("音频播放失败"));
          const started = player.play();
          if (started && typeof started.catch === "function") started.catch((e) => fail(e));
        } catch (e) { fail(e); }
      }, fail);
    }
    // 浏览器 TTS。它会偶发“吞掉 utterance / 不触发 onstart”，
    // 因此加 1 秒看门狗：没真正开始就交给 fallback（提示音）。
    function speakBrowser(text, done, fallback) {
      let started = false;
      let finished = false;
      let fellBack = false;
      ...
      try {
        if (typeof window === "undefined" || !window.speechSynthesis || !window.SpeechSynthesisUtterance) { fallbackOnce(); return; }
    ...
    function apply(ctx) {
      ...
      function SettingsPanel(props) {
        ...
        return react.createElement("div", { style: cardStyle },
          ...
          react.createElement("div", { style: rowStyle },
            react.createElement("span", { style: keyStyle }, t("engine")),
            react.createElement("select", { value: s.voiceEngine || "edge", onChange: e => commit(Object.assign({}, s, { voiceEngine: e.target.value })), style: selStyle },
              react.createElement("option", { value: "edge" }, t("engine.edge")),
              react.createElement("option", { value: "browser" }, t("engine.browser")))),
          react.createElement("div", { style: rowStyle },
            react.createElement("span", { style: keyStyle }, t("edge.voice")),
            react.createElement("select", { value: s.edgeVoice || "auto", onChange: e => commit(Object.assign({}, s, { edgeVoice: e.target.value })), style: selStyle },
              react.createElement("option", { value: "auto" }, t("edge.auto")),
              EDGE_VOICES.map(v => react.createElement("option", { key: v.id, value: v.id }, resolveLang() === "en" ? v.en : v.zh))),
            react.createElement("button", { style: btnStyle, onClick: previewEdgeVoice }, t("preview"))),
          (s.voiceEngine === "edge" && edgeDisabled)
            ? react.createElement("div", { style: hintStyle }, t("edge.unavailable") + (edgeStatus ? "（" + edgeStatus + "）" : ""))
            : null,
          (s.voiceEngine === "edge" && !edgeDisabled) ? react.createElement("div", { style: hintStyle }, t("edge.hint")) : null,
          react.createElement("div", { style: rowStyle },
