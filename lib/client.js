...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULTS = { volume: 0.7, types: DEFAULT_TYPES };
    ...
    function apply(ctx) {
      ...
      function settleRun(id) {
        ...
        ctx.timeout(() => {
          ...
          if (shouldFire(id, kind)) { playType(kind); emit(kind); }
          settling.delete(id);
        }, 250);
      }
      function observe(list) {
        // Only the session the user is looking at (no subagent / background noise).
        const byId = (list && list.byId) || {};
        ...
        if (p) {
          if (pending !== undefined && pending !== p.pending) {
            if (pending === "approval" && shouldFire(id, "approval")) { playType("approval"); emit("approval"); }
            else if (pending === "question" && shouldFire(id, "question")) { playType("question"); emit("question"); }
          }
